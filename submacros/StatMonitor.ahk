...
for v in ["haste","melody","redboost","blueboost","whiteboost","focus","bombcombo","balloonaura","clock","jbshare","babylove","inspire","bear","pollenmark","honeymark","festivemark","popstar","comforting","motivating","satisfying","refreshing","invigorating","blessing","bloat","guiding","mondo","reindeerfetch","tideblessing","blueflower","bamboo","pinetree"]
	buff_values[v] := Map()
...
graph_regions := Map("honey/sec", [regions["honey/sec"][1]+320,regions["honey/sec"][2]+130,3600,800]
	...
	, "pinetree", [regions["buffs"][1]+320,regions["buffs"][2]+3435,3600,110]
	, "bamboo", [regions["buffs"][1]+320,regions["buffs"][2]+3565,3600,110]
	, "blueflower", [regions["buffs"][1]+320,regions["buffs"][2]+3695,3600,110]
...
DetectBuffs()
{
	...
	for v in ["pinetree","bamboo","blueflower"]
		buff_values[v][i] := nm_fieldBoostCheck(v)
...
nm_fieldBoostCheck(fieldName){
	...
	pBMScreen:=Gdip_BitmapFromScreen(windowX "|" windowY + 73 "|" windowWidth "|" 48)
	result :=Gdip_ImageSearch(pBMScreen, bitmaps["boost"][fieldName "0"],,,,,,35) || 
		   Gdip_ImageSearch(pBMScreen, bitmaps["boost"][fieldName "1"],,,,,,35) || 
		   Gdip_ImageSearch(pBMScreen, bitmaps["boost"][fieldName "3"],,,,,,50)
	...
	; 			{
	; 				Gdip_DisposeImage(pBMScreen)
	; 				MsgBox "Booster detected: " fieldName " variant " variant " at position " ico
	; 				return 1 ; booster
	; 			}	
	; 	}
	; }
...
* @author SP
********************************************************************************************************/
SendHourlyReport(keepStats := false)
{
	...
	for k,v in graph_regions
	{
		...
		switch k
		{
			...
			case "festivemark","popstar","melody","bear","babylove","jbshare","guiding","pinetree","bamboo","blueflower":
			color := (k = "festivemark") ? 0xffc84335
				...
				: (k = "pinetree") ? 0xff6b5127
				: (k = "bamboo") ? 0xff1c8737
				: (k = "blueflower") ? 0xff0362fc
				: 0xffffef8e
	...
	try
	{
		...
		wr := ComObject("WinHttp.WinHttpRequest.5.1")
		wr.Option[9] := 2720
		wr.Open("POST", (discordMode = 0) ? webhook : ("https://discord.com/api/v10/channels/" 1460124708677619875 "/messages"), 0) ; hardcoded main channel id cuz i was lazy to figure out a way to get the var from settings
		if (discordMode = 1)
		{
			wr.SetRequestHeader("User-Agent", "DiscordBot (AHK, " A_AhkVersion ")")
			wr.SetRequestHeader("Authorization", "Bot " bottoken)
